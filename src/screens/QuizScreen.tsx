...
export function QuizScreen(props: { navigate: (path: RoutePath) => void; data: AppData["quiz"] }) {
  ...
  const next = async () => {
    ...
    if (index() === quiz.questions.length - 1) {
      setFinished(true);
      await recordAttempt({ quizSetId: quiz._id, score: score(), total: quiz.questions.length });
      return;
    }
  ...
  const restart = () => { setIndex(0); setSelected(null); setResult(undefined); setScore(0); setFinished(false); };
        ...
        <div class="quiz-layout">
          <main class="main-column"><Card class="quiz-stage">
            <Show when={!finished()} fallback={<div class="quiz-results"><div class="result-burst">★</div><p class="eyebrow">Quiz complete</p><h1>{score()} / {total()}</h1><p>{score() / Math.max(1, total()) >= .8 ? "Outstanding work—you’ve mastered this topic." : score() / Math.max(1, total()) >= .6 ? "Nice progress. Review the explanations and try once more." : "Good first step. Revisit the study explanation before another try."}</p><div class="result-actions"><Button onClick={restart}>Try again</Button><Button variant="secondary" onClick={() => props.navigate("/performance")}>View performance</Button></div></div>}>
              ...
              <Show when={result()}>{(feedback) => <div class={`answer-feedback ${feedback().correct ? "success" : "error"}`}><strong>{feedback().correct ? "Correct!" : "Not quite."}</strong><p>{feedback().explanation}</p></div>}</Show>
              <div class="quiz-actions"><span class="flex-1" />{result() ? <Button onClick={() => void next()}>{index() === total() - 1 ? "See results" : "Next question"} →</Button> : <Button disabled={selected() === null || checking()} onClick={() => void submit()}>{checking() ? "Checking…" : "Check answer"}</Button>}</div>
            </Show>
          </Card></main>
          <aside class="right-rail">
            <Card class="timer-card"><div class="section-heading compact"><div><h2>Time guide</h2><p>{quiz().durationMinutes} minutes total</p></div><IconBadge icon="◷" /></div><div class="timer-ring"><strong>{Math.max(1, Math.round(quiz().durationMinutes * 60 / Math.max(1, total())))}</strong><small>sec / question</small></div></Card>
